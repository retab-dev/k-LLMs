...
def _build_reference_list(sim_cache: SimilarityCache, client: OpenAI, min_support_ratio: float = 0.5, max_novelty_ratio: float = 0.5, threshold: float = 0.4) -> list[Index]:
    """
    ...
    for list_idx1, obj_pos1 in candidate_elements:
        ...
        if best_group_repr_index is not None:
            ...
            new_group_repr_index, _ = consensus_as_primitive(support_groups[best_group_repr_index], ConsensusSettings(), sync_get_openai_embeddings_from_text=dummy_embeddings_fn, client=client)
...
def lists_alignment(
    list_of_lists: list[list[Any]], sim_fn: Callable[[Any, Any], float], client: OpenAI, min_support_ratio: float = 0.5, max_novelty_ratio: float = 0.25, reference_list_idx: Optional[int] = None, 
    ...
    if reference_list_idx is None:
        ...
        reference_list = _build_reference_list(sim_cache, client, min_support_ratio, max_novelty_ratio, threshold=dynamic_threshold)
    ...
    if first_type is list:
        ...
        if any(lst for lst in lists_only):
            ...
            aligned_lists_only, original_list_reference_indices = lists_alignment(
                lists_only, sim_fn, client, min_support_ratio=min_support_ratio, max_novelty_ratio=max_novelty_ratio, reference_list_idx=reference_idx
            )
...
def string_consensus_llm(values: list[str], client: OpenAI) -> str:
    ...
    if response.choices[0].message.content is None:
        print("Warning: No content returned from LLM")
        return values[0]
    consensus_string = str(response.choices[0].message.content).strip()
    return consensus_string


async def async_string_consensus_llm(values: list[str], client: AsyncOpenAI) -> str:
    """Build a consensus string using an OpenAI client.
...
async def async_consensus_as_primitive(
    ...
    if first_val_type is str and consensus_settings.string_consensus_method == "llm-consensus" and consensus_settings.string_similarity_method == "embeddings":
        # Feed an LLM with the values and ask it to build the consensus string
        consensus_string = await async_string_consensus_llm(non_none_values, client=client)
        # Compute the similarity between the consensus string and the values
        similarities = []
...
async def async_lists_alignment(
    ...
    if reference_list_idx is None:
        ...
        # Get reference list and dynamically computed threshold
        reference_list = await _async_build_reference_list(
            async_sim_cache, 
            client,
            min_support_ratio, 
            max_novelty_ratio, 
...
async def _async_build_reference_list(
    ...
    for list_idx1, obj_pos1 in candidate_elements:
        ...
        if best_group_repr_index is not None:
            ...
            new_group_repr_index, _ = await async_consensus_as_primitive(
                support_groups[best_group_repr_index], ConsensusSettings(), 
                async_get_openai_embeddings_from_text=dummy_embeddings_fn,
                client=client
            )
            if new_group_repr_index != best_group_repr_index:
                support_groups[new_group_repr_index] = support_groups[best_group_repr_index]
