...
async def async_consolidate_chat_completions(
    ...
    # Check if we have multiple choices (from n parameter)
        ...
        for choice in completion.choices:
            if choice.message.content:
                all_contents.append(json.loads(choice.message.content))

        consensus_content, likelihoods = await async_consensus_values(all_contents, consensus_settings, async_get_openai_embeddings_from_text, is_last_chunk=True)
...
async def async_consolidate_parsed_chat_completions(
    ...
    for choice in completion.choices:
        if choice.message.parsed:
            # Convert parsed object to dict for consensus processing
            if hasattr(choice.message.parsed, "model_dump"):
                parsed_dict = choice.message.parsed.model_dump()
            elif hasattr(choice.message.parsed, "dict"):
                parsed_dict = choice.message.parsed.dict()
            else:
                # Fallback for other types
                parsed_dict = choice.message.parsed.__dict__
            all_parsed_objects.append(parsed_dict)
        else:
            all_parsed_objects.append(None)
    ...
    # Create consolidated usage (no need to consolidate since it's a single completion)
    consolidated_usage = completion.usage

    # Use the completion as template
    template = completion
